...
# Seleccionar el .deb según versión de Ubuntu
if [[ "$VERSION_ID" == "22.04" ]]; then
    PACKAGE_NAME="zentra-agent_${VERSION}_ubuntu22_amd64.deb"
elif [[ "$VERSION_ID" == "24.04" ]]; then
    PACKAGE_NAME="zentra-agent_${VERSION}_amd64.deb"
else
    echo -e "${YELLOW}[WARN]${NC} Ubuntu $VERSION_ID no probado, intentando con paquete 22.04..."
    PACKAGE_NAME="zentra-agent_${VERSION}_ubuntu22_amd64.deb"
fi

echo -e "${GREEN}[OK]${NC} Paquete seleccionado: $PACKAGE_NAME"
...
if systemctl is-active --quiet zentra-agent; then
    ...
    echo -e "${RED}[ERROR]${NC} El servicio no arrancó. Revisa: journalctl -u zentra-agent -n 50"
    exit 1
fi
